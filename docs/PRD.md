...
| 能力 | 实现 | 备注 |
| --- | --- | --- |
| 粘贴图片 | 编辑器 `handlePaste` 拦截 clipboard files → `POST /uploads` → 插入 image node | 剩下交给 Tiptap |
| 拖拽图片 | `handleDrop` 同样路径 | 与粘贴共用代码 |
| `/image` slash 命令 | `<input type=file accept=image/*>` 动态创建、click() | 不依赖其他 UI |
| 存储驱动 | `STORAGE_DRIVER` env（默认 `local`），预留 `s3` adapter | S3 SDK v0.3 不打进镜像 |
| 本地路径 | `${UPLOAD_DIR}/<2-char-prefix>/<sha256>.<ext>`（默认 `/data/uploads`） | content-addressed |
| 去重 | `(owner_id, sha256)` 唯一，同人重传同文件返回原有 id | 省磁盘 / 省带宽 |
| 大小限制 | `MAX_UPLOAD_SIZE` env，默认 25 MB | content-length + post-parse 双重检 |
| 下载 | `GET /uploads/:id` 返 stream + `Cache-Control: immutable` | id 是能力式访问 | 
| Markdown 导出 | `![alt](/uploads/<id>)` 相对路径 | 本地预览可用 |
| PDF 打印 | 导出时将 `/uploads/*` 重写为绝对 URL | 弹窗能加载 |
...
- **uploads**: `id, owner_id, content_hash, size, mime, original_name, storage_key, driver, created_at`
...
| `POST /uploads` (auth), `GET /uploads/:id` | 附件 |
